 ...
 */
export function detectVisionFromId(id: string): boolean {
  ...
  if (
    /audio|tts|asr|transcribe|livetranslate|realtime|embed|rerank|tokenizer/i.test(normalized) &&
...
function strIncludesVision(value: any): boolean {
 ...
 */
export function detectVisionFromMetadata(m: any): boolean {
  if (!m || typeof m !== 'object') {
    return false;
  }

  const labels: any[] = m.labels || m.tags || [];
  ...
  const arch: any = m.architecture || m.model_architecture || undefined;
  if (arch) {
 ...
 */
export function detectVisionSupport(m: any): boolean {
  const id =
    typeof m === 'string' ? m : m?.id || m?.name || m?.model || m?.checkpoint || '';
...
export function isAnyDataPart(part: any): boolean {
  return (
    !!part &&
    typeof part === 'object' &&
...
export function isImageDataPart(part: any): boolean {
  return (
    isAnyDataPart(part) &&
    (part as any).mimeType.toLowerCase().startsWith('image/')
  );
}

/** Encodes a data part as a `data:` URL suitable for OpenAI `image_url`. */
export function dataPartToDataUrl(part: { mimeType: string; data: Uint8Array }): string {
...
export function toOpenAIImagePart(dataUrl: string): any {
