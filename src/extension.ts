...
export function activate(context: vscode.ExtensionContext) {
  ...
  const treeDataProvider = new SimpleSignalTreeDataProvider(context.extensionUri);
  ...
  // 1. Set / Edit API Key
  const setApiKeyCmd = vscode.commands.registerCommand('simplesignal.endpoint.setApiKey', async (arg?: any) => {
  ...
  // 2. Edit Base URL
  const editUrlCmd = vscode.commands.registerCommand('simplesignal.endpoint.editUrl', async (arg?: any) => {
    const ep = await resolveEndpoint(arg);
    if (!ep) return;
    ...
    if (!newUrl || newUrl.trim() === ep.baseUrl) return;
  ...
  const toggleEndpointCmd = vscode.commands.registerCommand('simplesignal.endpoint.toggle', async (arg?: any) => {
    const ep = await resolveEndpoint(arg);
    if (!ep) return;
  ...
  const fetchEndpointModelsCmd = vscode.commands.registerCommand('simplesignal.endpoint.fetchModels', async (arg?: any) => {
    const ep = await resolveEndpoint(arg);
    if (!ep) return;
  ...
  // 5. Add Custom Model
  const addModelCmd = vscode.commands.registerCommand('simplesignal.endpoint.addModel', async (arg?: any) => {
    const ep = await resolveEndpoint(arg);
    if (!ep) return;
  ...
  const testConnCmd = vscode.commands.registerCommand('simplesignal.endpoint.testConnection', async (arg?: any) => {
    const ep = await resolveEndpoint(arg);
    if (!ep) return;
  ...
  const benchEndpointCmd = vscode.commands.registerCommand('simplesignal.endpoint.benchmark', async (arg?: any) => {
    const ep = await resolveEndpoint(arg);
    if (!ep) return;
    ...
    const picked = await vscode.window.showQuickPick(
      models.map((m) => ({ label: `$(zap) ${m.id}`, modelId: m.id })),
      { title: `Select Model to Benchmark on "${ep.name}"` }
    );
    if (!picked) return;
  ...
  const configureEndpointCmd = vscode.commands.registerCommand('simplesignal.endpoint.configure', async (arg?: any) => {
    const ep = await resolveEndpoint(arg);
    if (!ep) return;

    const protocols = ['openai', 'ollama', 'lemonade', 'anthropic', 'gemini'];
    const chosenProtocol = await vscode.window.showQuickPick(
      protocols.map((p) => ({
    ...
    if (!chosenProtocol) return;

    await updateEndpointInConfig(ep.name, ep.baseUrl, (target) => {
      target.protocol = chosenProtocol.protocol as any;
    });
  ...
  const copyJsonCmd = vscode.commands.registerCommand('simplesignal.endpoint.copyJson', async (arg?: any) => {
    const ep = await resolveEndpoint(arg);
    if (!ep) return;
  ...
  // 10. Delete Endpoint
  const deleteEndpointCmd = vscode.commands.registerCommand('simplesignal.endpoint.delete', async (arg?: any) => {
    const ep = await resolveEndpoint(arg);
    if (!ep) return;

    const confirm = await vscode.window.showWarningMessage(
      `Are you sure you want to delete endpoint "${ep.name}" (${ep.baseUrl})?`,
      { modal: true },
      'Yes, Delete Endpoint'
    );
    if (confirm !== 'Yes, Delete Endpoint') return;
...
async function resolveEndpoint(arg?: any): Promise<EndpointConfig | undefined> {
  if (arg && arg.endpoint) return arg.endpoint;
  if (arg && arg.baseUrl && arg.name) return arg as EndpointConfig;
...
async function updateEndpointInConfig(
  epName: string,
  epUrl: string,
  mutator: (target: EndpointConfig) => void
): Promise<void> {
  const config = vscode.workspace.getConfiguration('simplesignal');
