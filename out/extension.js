...
function activate(context) {
    ...
    const chatParticipant = chatParticipant_1.SimpleSignalChatParticipant.register(context, provider, treeDataProvider, statusBarItem, outputChannel);
    ...
    // Load model command
    const loadModelCmd = vscode.commands.registerCommand('simplesignal.loadModel', async (node) => {
    ...
    const unloadModelCmd = vscode.commands.registerCommand('simplesignal.unloadModel', async (node) => {
        let endpointName = node?.endpointName;
        let modelId = node?.model?.id || node?.label;
        if (!endpointName || !modelId) {
            const config = vscode.workspace.getConfiguration('simplesignal');
